...
# Layer with edges but without edge features updates
class MultiHeadAttentionEdgesCustom(nn.Module):
    def __init__(self, n_hidden, n_head, dropout):
        super(MultiHeadAttentionEdgesCustom, self).__init__()
        self.n_head = n_head
        self.n_hidden = n_hidden
        self.dim_head = n_hidden//n_head
...
class TransformerLayerEdgesCustom(nn.Module):
    def __init__(self, n_hidden, n_head, n_feedforward, dropout, norm):
        super(TransformerLayerEdgesCustom, self).__init__()
        self.n_hidden = n_hidden
        self.norm = norm
        # Multi-head Attention
        self.multihead_attention = MultiHeadAttentionEdgesCustom(n_hidden, n_head, dropout=dropout)
    ...
    def forward(self, x, e, attention_mask, padding_mask):
        ...
        # Self-attention
        x, scores = self.multihead_attention(x, e, attention_mask, padding_mask)
